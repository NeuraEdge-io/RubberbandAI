...
const OPT_STRATEGIES = [“High IV Crush (Sell)”,“Low IV Buy”,“Momentum Calls”,“Protective Puts”,“Covered Calls”,“Cash-Secured Puts”,“Debit Spreads”,“Iron Condors”,“Directional (Calls)”,“Directional (Puts)”];
...
function generateOptionContract(ticker, stockPrice, iv, optType, expLabel, strategy, idx) {
const isCall = optType === “call”;
const dte = parseInt(expLabel.match(/\d+/)?.[0] || “30”);
const t = dte / 365;

// Strike selection — ATM, slight OTM, deeper OTM
const strikes = [
...
// IV surface — higher iv for shorter dte, lower for longer
const ivAdj = iv / 100 * (1 + (0.1 * (7 / Math.max(dte, 1))));
const ivPct = Math.min(ivAdj, 2.5);

// Black-Scholes approximation for premium
const r = 0.053;
const d1 = (Math.log(stockPrice/strike) + (r + 0.5*ivPct*ivPct)*t) / (ivPct*Math.sqrt(t));
const d2 = d1 - ivPct*Math.sqrt(t);
const N = x => 0.5*(1+erf(x/Math.sqrt(2)));
function erf(x){const a1=0.254829592,a2=-0.284496736,a3=1.421413741,a4=-1.453152027,a5=1.061405429,p=0.3275911;const sign=x<0?-1:1;const t2=1/(1+p*Math.abs(x));const y=1-(((((a5*t2+a4)*t2)+a3)*t2+a2)*t2+a1)*t2*Math.exp(-x*x);return sign*y;}

let premium;
if(isCall) premium = Math.max(0.01, stockPrice*N(d1) - strike*Math.exp(-r*t)*N(d2));
else premium = Math.max(0.01, strike*Math.exp(-r*t)*N(-d2) - stockPrice*N(-d1));
premium = parseFloat(premium.toFixed(2));

// Greeks
const phi = x => Math.exp(-0.5*x*x)/Math.sqrt(2*Math.PI);
const delta = isCall ? N(d1) : N(d1) - 1;
const gamma = phi(d1) / (stockPrice * ivPct * Math.sqrt(t));
const theta = (-(stockPrice * phi(d1) * ivPct) / (2*Math.sqrt(t)) - r*strike*Math.exp(-r*t)*(isCall?N(d2):N(-d2))) / 365;
const vega = stockPrice * phi(d1) * Math.sqrt(t) / 100;
...
const contractLabel = `${ticker} $${strike} ${isCall?"CALL":"PUT"} ${expLabel.split(" ")[0]}`;
const moneyLabel = isATM ? “ATM” : isOTM ? “OTM” : “ITM”;

return {
ticker, name: OPTION_TICKERS.find(x=>x.t===ticker)?.n || ticker,
...
signal, contractLabel, moneyLabel,
...
const STEPS = [“Fetching option chain”,“Calculating fair value”,“Computing Greeks (Δ Γ Θ V ρ)”,“Analyzing IV surface”,“Generating entry points”,“Building AI insights”];
for(let i=0;i<STEPS.length;i++){await new Promise(r=>setTimeout(r,420));if(runId.current!==rid)return;setOStep(i+1);setOProgress(Math.round(((i+1)/STEPS.length)*82));}

```
const types = optType==="Calls Only"?["call"]:optType==="Puts Only"?["put"]:["call","put"];
const contracts=[];
types.forEach(tp=>{for(let i=0;i<3;i++)contracts.push(generateOptionContract(tickerData.t,tickerData.p,tickerData.iv,tp,optExp,optStrat,i));});
contracts.sort((a,b)=>b.delta*Math.sign(b.delta===a.delta?0:1)-(a.delta*Math.sign(1)));
...
if(runId.current!==rid)return;
setOProgress(100); await new Promise(r=>setTimeout(r,180));
...
const OPT_STEPS=[“Fetching option chain”,“Calculating fair value”,“Computing Greeks (Δ Γ Θ V ρ)”,“Analyzing IV surface”,“Generating entry points”,“Building AI insights”];
